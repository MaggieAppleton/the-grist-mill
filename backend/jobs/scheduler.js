...
function initializeScheduler() {
	...
	const enableNightlyRerank = String(process.env.ENABLE_NIGHTLY_RERANK || 'false') === 'true';
	...
	if (enableNightlyRerank) {
		cron.schedule(rerankCron, async () => {
			console.log(`[${new Date().toISOString()}] Nightly rerank job triggered (cron="${rerankCron}")`);
			try {
				const result = await runRerank({ force: true, batchSize: rerankBatchSize });
				if (result.ok) {
					console.log(`[${new Date().toISOString()}] Nightly rerank started successfully`);
				} else {
					console.warn(`[${new Date().toISOString()}] Nightly rerank skipped/failed to start: ${result.error}`);
				}
			} catch (error) {
				console.error(`[${new Date().toISOString()}] Nightly rerank error:`, error);
			}
		});
	}
	...
	if (enableNightlyRerank) {
		console.log(`[${new Date().toISOString()}] Scheduler: Nightly rerank enabled (cron="${rerankCron}")`);
	} else {
		console.log(`[${new Date().toISOString()}] Scheduler: Nightly rerank disabled (set ENABLE_NIGHTLY_RERANK=true to enable)`);
	}
}
